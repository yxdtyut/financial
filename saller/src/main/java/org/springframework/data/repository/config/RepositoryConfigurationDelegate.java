...
public class RepositoryConfigurationDelegate {
    ...
    public List<BeanComponentDefinition> registerRepositoriesIn(BeanDefinitionRegistry registry,
        ...
        for (RepositoryConfiguration<? extends RepositoryConfigurationSource> configuration : extension
            ...
            AnnotationMetadata annotationMetadata = (AnnotationMetadata) configurationSource.getSource();
            //判断配置类是否使用primary标注，如果有，就设为primary
            if (annotationMetadata.hasAnnotation(Primary.class.getName())) {
                beanDefinition.setPrimary(true);
            } else if (annotationMetadata.hasAnnotation(RepositoryBeanNamePrefix.class.getName())) {
                Map<String,Object> prefixData = annotationMetadata.getAnnotationAttributes(RepositoryBeanNamePrefix.class.getName());
                String prefix = (String) prefixData.get("value");
                beanName = prefix + beanName;
            }
            //再判断有没有使用RepositoryBeanNamePrefix进行标注，如果有，添加名称前缀
